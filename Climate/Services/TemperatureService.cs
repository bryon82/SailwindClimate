...
namespace Climate
{
    internal static class TemperatureService
    {
        ...
        const float ReferenceLatitude = 31f;
        const float ReferenceTemperature = 30f;
        const float TempLatConversionFactor = 1.2f;

        internal static float GetTemperature(Vector3 coords, float time, int day)
        {
            var baseTemp = ReferenceTemperature - (coords.z - ReferenceLatitude) * TempLatConversionFactor;
            ...
            return baseTemp + seasonal + diurnal + noise + lowPressure;
        }
